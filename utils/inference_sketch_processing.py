...
def make_graph(sketch, graph_num=30, graph_picture_size=128, padding=0, thickness=5,
    ...
    canvas_target = cv2.resize(canvas[boundary_size:boundary_size + _h, boundary_size:boundary_size + _w],
                              (48,48))
    ...
    if graph_count + 1 < graph_num:
        ...
        adj_matrix[:, graph_count + 1 + 1:] = 0
    sketch_idx+=1
    return patch_trans(canvas_target).unsqueeze(0).repeat(2,1,1,1), adj_matrix
