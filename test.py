...
class Model:
    ...
    def train(self, epoch):
        ...
        # update eta for LKL:
        self.eta_step = 1. - 0.999 * (0.9999 ** epoch)  # self.eta_step = 1 - (1 - hp.eta_min) * hp.R
        # compute losses:
        # LKL = self.kullback_leibler_loss()
        LR = self.reconstruction_loss(mask, dx, dy, p, epoch)
        # loss = LR + LKL
        loss = LR + rpclloss*self.eta_step + 0.5*mseloss
        # gradient step
...
if __name__ == "__main__":
    ...
    for epoch in range(500001):
        ...
        #print(model.encoder.de_alpha)
            print(model.encoder.de_mu.data.mean(dim=-1))
            print(model.encoder.de_sigma2.mean(dim=-1))
            print(sorted(model.encoder.de_alpha.data))
            break
        model.train(epoch)
