...
class myencoder(nn.Module):
    ...
    def forward(self, graph):
        ...
        self.p_alpha, self.gau_label = self.calculate_posterior(z, self.de_mu, self.de_sigma2, self.de_alpha)

        #print(self.p_alpha,self.gau_label)
        if self.global_ > hp.br+500:
            if (self.global_ % 3) == 0:
                q_mu, q_sigma2, q_alpha = self.em(self.p_mu, self.p_sigma2, self.p_alpha, self.de_mu,
                                                                 self.de_sigma2, self.de_alpha)
            else:
                q_mu, q_sigma2, q_alpha = self.rpcl(self.p_mu, self.p_sigma2, self.p_alpha, self.de_mu,
                                                                 self.de_sigma2, self.de_alpha)
        else:
    ...
    def calculate_prob(self, x, q_mu, q_sigma2):
        ...
        log_exp_part = -0.5 * torch.sum(torch.div(torch.square(x - mu), 1e-30 + sigma2), dim=2).cuda()
        log_frac_part = torch.sum(torch.log(torch.sqrt(sigma2 + 1e-30)), dim=2).cuda()
        log_prob = log_exp_part - log_frac_part - float(self.z_size) / 2. * torch.from_numpy(
            np.log([2. * 3.1416])).cuda()
